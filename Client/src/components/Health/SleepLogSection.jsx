import { Plus, X, Moon, Clock, Edit2, Trash2 } from "lucide-react";
        ...
        <div className="p-5 rounded-2xl border" style={{ background: "var(--card-bg)", borderColor: "var(--card-border)" }}>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              ...
              <input type="datetime-local" className="w-full px-3 py-2 rounded-xl text-sm outline-none transition-all"
                style={{ background: "var(--hover-bg)", color: "var(--text-primary)", border: "1px solid var(--card-border)" }}
                value={form.start_time} onChange={(e) => onFormChange("start_time", e.target.value)} />
            </div>
            <div>
              <label className="block text-xs mb-1.5" style={{ color: "var(--text-secondary)" }}>Sleep End</label>
              <input type="datetime-local" className="w-full px-3 py-2 rounded-xl text-sm outline-none transition-all"
                style={{ background: "var(--hover-bg)", color: "var(--text-primary)", border: "1px solid var(--card-border)" }}
                value={form.end_time} onChange={(e) => onFormChange("end_time", e.target.value)} />
            </div>
          </div>
          <div className="mb-4">
            ...
            <div className="flex gap-2">
              {["poor", "fair", "good", "excellent"].map((q) => {
                ...
                return (
                  <button key={q} type="button" onClick={() => onFormChange("quality", q)}
          ...
          <div className="mb-4">
            ...
            <input className="w-full px-3 py-2 rounded-xl text-sm outline-none transition-all"
              style={{ background: "var(--hover-bg)", color: "var(--text-primary)", border: "1px solid var(--card-border)" }}
              placeholder="Any observations..." value={form.notes} onChange={(e) => onFormChange("notes", e.target.value)} />
          </div>
          <button className="px-5 py-2 rounded-xl text-sm font-semibold text-white transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-40"
            ...
            <div key={log.id} className="p-4 rounded-2xl border flex items-center gap-4" style={{ background: "var(--card-bg)", borderColor: "var(--card-border)" }}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs mt-0.5 truncate" style={{ color: "var(--text-muted)" }}>
                  {log.start_time ? new Date(log.start_time).toLocaleString() : "N/A"} — {log.end_time ? new Date(log.end_time).toLocaleTimeString() : "N/A"}
                </p>
                {log.notes && <p className="text-xs mt-1 italic" style={{ color: "var(--text-secondary)" }}>{log.notes}</p>}
              </div>

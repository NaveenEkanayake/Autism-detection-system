    ...
    <div ref={containerRef} className={`relative w-full ${embedded ? "h-full" : "bg-[#030509]"} overflow-hidden font-sans`} style={embedded ? undefined : { height: "100vh" }}>
      ...
      {!embedded && (
        <>
          ...
          <div className="absolute inset-0 pointer-events-none z-0 opacity-20" style={{ backgroundSize: "80px 80px", backgroundImage: "linear-gradient(to right, rgba(255,255,255,0.015) 1px, transparent 1px), linear-gradient(to bottom, rgba(255,255,255,0.015) 1px, transparent 1px)", maskImage: "radial-gradient(ellipse at center, black 0%, transparent 75%)", WebkitMaskImage: "radial-gradient(ellipse at center, black 0%, transparent 75%)" }} aria-hidden="true" />
        </>
      )}
      ...
      <div ref={headerRef} className="absolute top-0 left-0 right-0 z-40 text-center pt-4 md:pt-10 pb-2 md:pb-6 px-4">
        ...
        <h2 className="text-lg md:text-4xl lg:text-5xl font-extrabold text-white tracking-tight mb-2 md:mb-4 leading-tight px-2">
          ...
          return (
            <div key={slide.id} data-slide={i} className="absolute inset-0 flex flex-col lg:flex-row items-center justify-center gap-4 md:gap-10 lg:gap-16 px-4 md:px-10 lg:px-16 overflow-y-auto pb-24">
              ...
              <div className="w-full lg:w-[52%] shrink-0 flex items-center justify-center">
                <div className="relative w-full max-w-[560px] aspect-[4/3] md:aspect-[16/11] rounded-xl md:rounded-3xl overflow-hidden shadow-2xl shadow-black/60 border border-white/5">
              ...
              <div className="parallax-fg w-full lg:w-[42%] max-w-lg flex flex-col justify-center will-change-transform gap-3 md:gap-6">
                ...
                <div>
                  <h3 className="text-white text-xl md:text-3xl lg:text-4xl font-bold tracking-tight mb-2 md:mb-3 leading-[1.15]">{slide.title}</h3>
                  <p className="text-neutral-300/90 text-xs md:text-base font-light leading-relaxed">{slide.description}</p>
                </div>
                ...
                <div className="space-y-2 md:space-y-3 pt-1 md:pt-2">
                  {slide.features.map((feature, idx) => (
                    <div key={idx} className="flex items-start gap-3">
                      ...
                      <p className="text-neutral-300 text-[11px] md:text-sm font-light leading-relaxed">{feature}</p>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <div className="absolute right-0 bottom-0 w-[40%] z-30 hidden lg:flex flex-col justify-center px-6 md:px-10 lg:px-12 overflow-hidden" style={{ top: "clamp(130px, 20vh, 210px)" }}>
        ...
        <div className="space-y-3">
          {ABOUT_SLIDES.map((slide, i) => {
            ...
            return (
              <div key={slide.id} className={`group relative flex items-center gap-4 p-4 md:p-5 rounded-2xl border transition-all duration-700 ease-out ${isActive ? "bg-gradient-to-r from-blue-600/10 to-transparent border-blue-500/30 shadow-lg shadow-blue-600/5" : "bg-white/[0.01] border-white/5 hover:border-white/10"}`}>
                <div className={`shrink-0 p-2.5 rounded-xl transition-all duration-700 ${isActive ? "bg-blue-600 text-white shadow-md shadow-blue-600/30 scale-110" : "bg-white/5 text-neutral-600 group-hover:bg-white/10"}`}>
                  <Icon className="size-4" strokeWidth={2} />
                </div>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 mb-0.5">
                    <span className={`text-[8px] font-bold uppercase tracking-[0.2em] transition-colors duration-700 ${isActive ? "text-blue-400" : "text-neutral-700"}`}>{slide.tag}</span>
                    <span className={`text-xs font-bold tracking-wide transition-colors duration-700 ${isActive ? "text-white" : "text-neutral-500 group-hover:text-neutral-400"}`}>{slide.label}</span>
                  </div>
                  <p className={`text-[11px] leading-relaxed transition-all duration-700 ${isActive ? "text-neutral-400 max-h-20 opacity-100" : "text-neutral-700 max-h-8 opacity-60 line-clamp-2"}`}>{slide.title}</p>
                </div>
      ...
      {/* Mobile Nav */}
      <div className="absolute bottom-0 left-0 right-0 z-40 lg:hidden bg-gradient-to-t from-black/95 via-black/70 to-transparent px-4 pb-6 pt-16">

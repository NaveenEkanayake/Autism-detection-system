        ...
        return (
          <div className="space-y-4">
            {["Social Smile", "Gaze Following", "Pointing", "First Words", "Joint Attention"].map((milestone, idx) => (
              <motion.div key={idx} className="p-4 rounded-lg bg-black/5 dark:bg-white/5 border border-blue-500/20 hover:opacity-80 transition-all" variants={itemVariants}>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <input type="checkbox" className="w-5 h-5 rounded border-blue-500" />
                    <span className="landing-text-primary font-medium">{milestone}</span>
                  </div>
                  <span className="text-xs landing-text-secondary">Age: 3-12m</span>
                </div>
              </motion.div>
            ))}
          </div>
            ...
            {[
              ...
              <motion.div key={idx} variants={itemVariants}>
                <div className="flex justify-between mb-2">
                  <span className="landing-text-primary font-medium">{item.label}</span>
                  <span className="text-sm landing-text-secondary">{item.value} ({item.percentile})</span>
                </div>
        ...
        return (
          <div className="space-y-4">
            {[
              ...
              <motion.div key={idx} className="p-4 rounded-lg bg-black/5 dark:bg-white/5 border border-indigo-500/20 hover:opacity-80 transition-all flex items-center justify-between" variants={itemVariants}>
                <div>
                  <p className="landing-text-primary font-medium">{log.date}</p>
                  <p className="text-sm landing-text-secondary">{log.hours}</p>
                </div>
    ...
    >
      <div className="max-w-5xl mx-auto">
        ...
        <motion.div className="text-center mb-12" variants={itemVariants}>
          ...
          <p className="landing-text-secondary">Track developmental progress with CDC standards</p>
        </motion.div>
          ...
          {HEALTH_TRACKER_ITEMS.map((item) => {
            ...
            return (
              <motion.button
                ...
                className={`p-6 rounded-xl transition-all text-left landing-card-border landing-card-glass ${
                  selectedTracker === item.id
                    ? "shadow-lg opacity-100 ring-1 ring-blue-500/30"
                    : "hover:opacity-90"
                }`}
              ...
              >
                ...
                <h3 className="landing-text-primary font-semibold">{item.title}</h3>
                <p className="text-sm landing-text-secondary mt-1">{item.description}</p>
              </motion.button>
            );
          })}
        </div>
